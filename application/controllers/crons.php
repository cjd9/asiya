...
class Crons extends CI_Controller
{
    ...
    function index()
	{
		...
		$data['logs'] = $this->db->query("SELECT `pk`, `patient_id`, `patient_name`, `title`, `sms`, `email`, `sms_sent`, `email_sent`, CONVERT_TZ(date_Created,'+00:00','+05:30') as date_created FROM cron_log WHERE  date_created >= DATE(NOW()) - INTERVAL 7 DAY order by date_created desc");
				...
				if($rsappointment->num_rows() > 0)
				{
					...
					if(!empty($row))		// check if existing patient, then take email id to send mail
					{
						...
						if($rspatient->num_rows() > 0)
						{
							...
							$html .= 'FOR ANY QUERIES  PLEASE CALL US ON 022 40067272 OR VIST OUR WEBSITE ASIYA.CO.IN <br><br><br><br>';
